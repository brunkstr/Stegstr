...
// Events must be genuinely signed: the pool verifies id and signature before
// dispatch (from #108) and drops anything forged, so a fake sig never arrives.
const TEST_SK = Nostr.generateSecretKey();
function noteEvent(content: string): NostrEvent {
  return Nostr.finishEvent({ kind: 1, content, tags: [], created_at: 1 }, TEST_SK) as NostrEvent;
}
...
describe("connectRelays", () => {
  ...
  it("delivers an event once even when several relays carry it", async () => {
    vi.useRealTimers(); // this test is about de-duplication, not timing; verification is async
    const onEvent = vi.fn();
    const pool = connectRelays([PUBKEY], onEvent, vi.fn(), vi.fn(), RELAYS);
    const flush = () => new Promise((r) => setTimeout(r, 20));

    const dup = noteEvent("dup-1");
    FakeWebSocket.instances.forEach((ws) => {
      ws.open();
      ws.emit(["EVENT", ws.feedSubId(), dup]);
    });
    await flush();
...
describe("publishEvent", () => {
  it("reports which relays accepted and which rejected", async () => {
    ...
    const [a, b, c] = FakeWebSocket.instances;
    a.open();
    a.emit(["OK", event.id, true, ""]);
    b.open();
    b.emit(["OK", event.id, false, "blocked: pow required"]);
    c.open();
